  ...
  # if wrong leader elected, Paxos will reject proposals, because not leader (if check)
    ...
    def init(name, participants) do
      state = %{
        ...
        parent_name: name, # update to be paxos process name
    ...
    defp elec_leader(state) do
      ...
      if MapSet.size(state.alive) > 0 do
        first_elem = Enum.at(Enum.sort(state.alive), 0) # sort the list and make the first element the leader
        if first_elem != state.leader do # if first elem is already leader dont elect new leader
